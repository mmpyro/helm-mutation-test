...
func TestCutDeclarations(t *testing.T) {
	tests := []struct {
		...
		// Starts with "$", unlike the case above, so the scanner does read a
		// variable name before deciding. It then stops at the "|" — a separator
		// that cannot appear in a declaration — and so never reaches the ":="
		// inside the literal. That is the property pinned here: a leading "$" is
		// not enough to make something a declaration.
